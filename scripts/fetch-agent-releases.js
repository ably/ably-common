...
if (!GITHUB_TOKEN) {
  console.warn('⚠️  Warning: GITHUB_TOKEN not set. GitHub API rate limits will be restrictive.');
  console.warn('   Set GITHUB_TOKEN environment variable for better rate limits.');
  console.warn('   The script will proceed but may hit rate limits with many releases.');
}

if (!OPENAI_API_KEY) {
  console.warn('⚠️  Warning: OPENAI_API_KEY not set. Release info summarization will be skipped.');
  console.warn('   Set OPENAI_API_KEY environment variable to enable AI summarization.');
    ...
    lines.slice(1).forEach(line => {
      if (line.trim()) {
        ...
        existingData[key] = {
          ...
          releaseInfoSummary: releaseInfoSummaryIndex >= 0 ? fields[releaseInfoSummaryIndex] : ''
        };
      }
    });
  }
      ...
      res.on('end', () => {
        try {
          ...
          if (responseData.trim().startsWith('<')) {
            console.error('  OpenAI API returned HTML error page (likely rate limited)');
            resolve('');
            return;
          }
          ...
          if (response.error) {
            console.error(`  OpenAI API error: ${response.error.message || response.error.type || 'Unknown error'}`);
            resolve('');
            return;
          }
          ...
          if (responseData.length > 100) {
            console.error('  Error parsing OpenAI response: Response too long, likely an error page');
          } else {
...
function getReleaseType(version) {
  ...
  if (normalized.includes('-rc') || normalized.includes('.rc') || normalized.includes('+rc') || 
      normalized.includes('release-candidate') || normalized.includes('-preview') || normalized.includes('+preview')) {
    return 'release-candidate';
  }
  if (normalized.includes('-beta') || normalized.includes('.beta') || normalized.includes('+beta') || 
      normalized.includes('-swift') || normalized.includes('+swift')) {
    return 'beta';
  }
  if (normalized.includes('-alpha') || normalized.includes('.alpha') || normalized.includes('+alpha') || 
      normalized.includes('-experiment') || normalized.includes('+experiment') || 
      normalized.includes('-dev') || normalized.includes('+dev')) {
    return 'alpha';
  }
  
  // Check for any other suffix
  if (normalized.includes('-') || normalized.includes('+')) {
    const suffix = normalized.split(/[-+]/)[1];
    if (suffix) {
      console.warn(`  ⚠️  Unknown version suffix in ${version}, categorizing as alpha`);
      return 'alpha';
    }
  }
 ...
 */
async function processReleases(agent, releases, latestVersion, latestVersionDate, owner, repo) {
  const rows = [];
  const sunsettedVersions = findSunsettedVersions(agent);
  
  // Find if there's a newer stable version for deprecation calculation
  const stableReleases = releases.filter(r => getReleaseType(r.tag_name) === 'stable' && !r.prerelease && !r.draft);
  ...
  for (const release of releases) {
    ...
    if (!releaseInfo && release.tag_name) {
      console.log(`  Fetching release info for ${agent.identifier} ${version}...`);
      ...
      if (releaseInfo && !releaseInfoSummary && OPENAI_API_KEY) {
        console.log(`  Generating AI summary for ${agent.identifier} ${version}...`);
...
async function main() {
  ...
  const agentsToProcess = agents.agents.filter(agent => 
  ...
  for (const [source, repoAgents] of Object.entries(repoToAgents)) {
    try {
      ...
      const sortedReleases = releases
        .filter(r => !r.prerelease && !r.draft && getReleaseType(r.tag_name) === 'stable')
        .sort((a, b) => compareVersions(b.tag_name, a.tag_name));
      
      const latestRelease = sortedReleases[0];
      const latestVersion = latestRelease?.tag_name || '';
      const latestVersionDate = latestRelease?.published_at ? latestRelease.published_at.split('T')[0] : '';
      console.log(`  Latest stable version: ${latestVersion}`);
      
      // Process releases for each agent using this repository
      for (const agent of repoAgents) {
        const agentRows = await processReleases(agent, releases, latestVersion, latestVersionDate, owner, repo);
        allRows.push(...agentRows);
        console.log(`  Processed ${agentRows.length} releases for ${agent.identifier}`);
      }
