...
title: Channel resumed in a different region
summary: The client reconnected to a different Ably region from the one it was using, so the position it resumed from could not be applied. The channel attaches at the current point, and no earlier messages are replayed.
---
...
The error is reported with code 90005 and HTTP status 404, with the message `Unable to recover channel (unable to resume from a different site)`. Here, "site" is the internal term for a region.
